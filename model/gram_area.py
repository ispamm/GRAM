...
class GRAM(MMGeneralModule):
    ...
    def forward_ret(self, batch, task, compute_loss=True):
        ...
        if compute_loss:
            ...
            with torch.no_grad():
                weights_t2cond = F.softmax(-(area-vid_sim), dim=1) + 1e-4
                weights_t2cond[:, rank * bs : rank * bs + bs].fill_diagonal_(0)
                weights_cond2t = F.softmax(-(areaT-vid_simT), dim=1) + 1e-4
